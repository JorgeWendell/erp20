...
export function ClientForm({ client, open: controlledOpen, onOpenChange, trigger }: ClientFormProps) {
  ...
  const { execute: createClient } = useAction(createClientAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
        toast.success("Cliente cadastrado com sucesso!");
        form.reset();
        setOpen(false);
        window.dispatchEvent(new Event("client-created"));
      }
    },
    onError: ({ error }) => {
  ...
  const { execute: updateClient } = useAction(updateClientAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
    ...
    onError: ({ error }) => {
      toast.error(error.serverError || "Erro ao atualizar cliente");
    },
    onExecute: () => {
      setIsSubmitting(true);
    },
  ...
  const dialogContent = (
    <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>{isEditMode ? "Editar Cliente" : "Cadastrar Cliente"}</DialogTitle>
        <DialogDescription>
          {isEditMode
